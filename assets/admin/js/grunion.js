...
FB.ContactForm = function() {
	...
	function sendShortcodeToEditor () {
		try {
			...
			jQuery.post(ajaxurl, fbForm, function(response) {
				...
				var currentCode = jQuery('#jetpack_form', window.parent.document).val();
				...
				if (currentCode.match(regexp)) {
					jQuery( '#jetpack_form', window.parent.document ).val( currentCode.replace( regexp, response ) );
				} else {
					jQuery( '#jetpack_form', window.parent.document ).val( currentCode + response );
				}
				hidePopup();
			});
		} catch(e) {
	...
	return {
		...
		init: function () {
			...
			var contentSource = jQuery('#jetpack_form', window.parent.document).val();
			var data = {
